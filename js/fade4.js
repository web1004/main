$(document).ready(function(){
  ...
  let $bgimg = $(".back_img ul li");
  let $btn = $(".btn ul li");
  let $lbtn = $(".side_btn .lbtn");
  let $rbtn = $(".side_btn .rbtn");
  let oldidx = 0; //기존이미지
  let idx = 0; //새로 바뀌는 이미지
  let imgCount = $img.length; // length메서드 -> 1,2,3...이미지의 개수를 세어서 변수를 저장
  ...
  function changeImg(idx){ //매개변수가 있는 함수 ->idx는 선택되는 이미지
    if(oldidx != idx){ //기존의 이미지와 선택된 이미지가 다를때...
      ...
      //가로100% 배경이미지 바뀌는 부분 추가
      $bgimg.eq(oldidx).stop().fadeOut("300"); 
      $bgimg.eq(idx).stop().fadeIn("300");
    }
  ...
  function changeAuto(){
    idx++;
    if(idx > imgCount-1){ //index는 0부터, length는 1부터 시작하므로 1을 빼주어야 마지막 숫자가 맞음
      idx=0;
    }
    changeImg(idx);
  };
  ...
  $rbtn.click(function(){
    clearInterval(timer);
    idx++;
    if(idx > imgCount-1){ //선택한 이미지가 0,1,2,3...4 마지막번째일때 맨 처음부터 다시 시작
      idx=0;
    }
    changeImg(idx);
    timer = setInterval(changeAuto,4000);
